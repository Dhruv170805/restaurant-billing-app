    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast.success(orderId ? `Items added to Order #${orderId}!` : `Order placed for Table ${tableNumber}!`)
        ...
        <div className="flex gap-6" style={{ height: 'calc(100vh - 120px)' }}>
            ...
            <div className="flex-1 flex flex-col gap-4 overflow-hidden">
                ...
                <div
                    ...
                    style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.65rem', alignContent: 'start' }}
                >
                    {filteredItems.length === 0 ? (
                        <div className="card" style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '3rem' }}>
                            <p style={{ color: 'var(--foreground-subtle)', fontSize: '1.1rem' }}>
                                {searchQuery ? 'No items match your search.' : 'No items found. Add some from the Menu page.'}
                            </p>
                        </div>
                    ) : (
                        filteredItems.map(item => {
                            ...
                            return (
                                <div
                                    ...
                                    style={{
                                        position: 'relative',
                                        background: inCart ? 'rgba(var(--primary-rgb, 99,102,241), 0.08)' : 'rgba(255,255,255,0.03)',
                                        border: inCart ? '1px solid rgba(var(--primary-rgb, 99,102,241), 0.25)' : '1px solid var(--glass-border)',
                                        borderRadius: '14px',
                                        padding: '0.9rem 1rem',
                                        cursor: 'pointer',
                                        display: 'flex',
                                    ...
                                    onMouseLeave={e => {
                                        ...
                                        e.currentTarget.style.borderColor = inCart ? 'rgba(var(--primary-rgb, 99,102,241), 0.25)' : 'var(--glass-border)'
                                    }}
                                >
                                    {/* Top accent */}
                                    <div style={{
            ...
            <div className="card flex flex-col" style={{ width: '380px', height: '100%' }}>
                <div style={{ borderBottom: '1px solid var(--glass-border)', paddingBottom: '1rem', marginBottom: '1rem' }}>
                    ...
                    <p style={{ color: 'var(--foreground-muted)', fontSize: '0.85rem', marginTop: '0.25rem' }}>
                        {cart.length === 0 ? 'No items yet' : `${cart.reduce((s, i) => s + i.quantity, 0)} items`}
                    </p>
                </div>
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="flex items-center gap-2" style={{
                                        ...
                                        <button
                                            ...
                                            style={{ fontSize: '1.1rem', color: 'var(--foreground-muted)', transition: 'all 0.15s' }}
                                    ...
                                    <button
                                        style={{ color: 'var(--danger)', fontSize: '1.2rem', background: 'none', border: 'none', cursor: 'pointer', transition: 'all 0.15s' }}
                    ...
                    {settings?.taxEnabled && tax > 0 && (
                        <>
                            <div className="flex justify-between items-center" style={{ marginBottom: '0.5rem' }}>
                                <span style={{ fontSize: '0.85rem', color: 'var(--foreground-muted)' }}>Subtotal</span>
