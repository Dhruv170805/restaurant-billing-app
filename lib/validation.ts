...
export function validateStringLength(
    ...
    if (clean.length < min) {
        throw new ValidationError(`${fieldName} must be at least ${min} character(s)`, { field: fieldName, min })
    }
    if (clean.length > max) {
        throw new ValidationError(`${fieldName} must be at most ${max} characters`, { field: fieldName, max })
    }
    return clean
}
...
export function validatePositiveNumber(val: unknown, fieldName: string): number {
    if (typeof val !== 'number' || !Number.isFinite(val) || val <= 0) {
        throw new ValidationError(`${fieldName} must be a positive number`, { field: fieldName, received: val })
    }
    return val
}
...
export function validateNonNegativeNumber(val: unknown, fieldName: string): number {
    if (typeof val !== 'number' || !Number.isFinite(val) || val < 0) {
        throw new ValidationError(`${fieldName} must be a non-negative number`, { field: fieldName, received: val })
    }
    return val
}
...
export function validatePositiveInteger(val: unknown, fieldName: string): number {
    if (typeof val !== 'number' || !Number.isInteger(val) || val <= 0) {
        throw new ValidationError(`${fieldName} must be a positive integer`, { field: fieldName, received: val })
    }
    return val
}
...
export function validateRange(
    ...
    if (val < min || val > max) {
        throw new ValidationError(`${fieldName} must be between ${min} and ${max}`, { field: fieldName, min, max, received: val })
    }
    return val
}
...
export function validateEnum<T extends string>(
    ...
    if (!allowed.includes(val as T)) {
        throw new ValidationError(
            ...
            { field: fieldName, allowed, received: val },
        )
    }
    return val as T
}
...
export function validateOrderItems(items: unknown): ValidatedCartItem[] {
    ...
    return items.map((item: RawCartItem, index: number) => {
        ...
        return {
            ...
            quantity: validateRange(item.quantity, `${prefix} quantity`, 1, MAX_QUANTITY_PER_ITEM) as number,
        }
    })
}
...
const KNOWN_CURRENCY_CODES = new Set([
    ...
    'TWD', 'DKK', 'PLN', 'THB', 'IDR', 'MYR', 'PHP', 'AED', 'SAR', 'BDT',
    'PKR', 'LKR', 'NPR',
])
