    ...
    try {
        ...
        for (const cat of data.categories) {
            const [result] = await conn.execute<mysql.ResultSetHeader>(
                'INSERT INTO categories (name) VALUES (?)', [cat.name]
            )
...
export async function addMenuItem(name: string, price: number, categoryId: number): Promise<MenuItem | null> {
    const pool = await ensureInitialized()
...
export async function updateMenuItem(id: number, updates: { name?: string; price?: number; categoryId?: number }): Promise<MenuItem | null> {
    const pool = await ensureInitialized()
...
export async function getOrdersPaginated(page: number = 1, limit: number = 20): Promise<PaginatedOrders> {
    const pool = await ensureInitialized()
    const offset = (page - 1) * limit

    const [totalRows] = await pool.execute<mysql.RowDataPacket[]>('SELECT COUNT(*) as total FROM orders')
 ...
 */
export async function createOrder(items: OrderItem[], _clientTotal: number, tableNumber: number): Promise<Order> {
    const pool = await ensureInitialized()
    const conn = await pool.getConnection()
    ...
    try {
        ...
        const [countRows] = await conn.execute<mysql.RowDataPacket[]>(
            'SELECT COUNT(*) as c FROM orders WHERE created_at >= ?', [startOfDayFormatted]
        )
        ...
        for (const item of items) {
            ...
            if (existingItemRows.length > 0) {
                ...
                await conn.execute(
                    ...
                    [newQty, item.price, existingItemRows[0].id]
                )
            } else {
                // Insert new item
                await conn.execute(`
...
export async function updateOrderStatus(id: number, status: 'PENDING' | 'PAID' | 'CANCELLED', paymentMethod?: 'CASH' | 'ONLINE'): Promise<Order | null> {
    const pool = await ensureInitialized()
    const now = new Date().toISOString().slice(0, 19).replace('T', ' ')

    if (paymentMethod) {
        const [result] = await pool.execute<mysql.ResultSetHeader>(
            'UPDATE orders SET status = ?, payment_method = ?, updated_at = ? WHERE id = ?', [status, paymentMethod, now, id]
        )
        if (result.affectedRows === 0) return null
    } else {
        const [result] = await pool.execute<mysql.ResultSetHeader>(
            'UPDATE orders SET status = ?, updated_at = ? WHERE id = ?', [status, now, id]
        )
        if (result.affectedRows === 0) return null
    }
...
export async function getDashboardStats() {
    ...
    const [todayRevenueRows] = await pool.execute<mysql.RowDataPacket[]>(
        "SELECT COALESCE(SUM(total), 0) as r FROM orders WHERE created_at >= ? AND status = 'PAID'", [startOfDayFormatted]
    )
    const [cashRevenueRows] = await pool.execute<mysql.RowDataPacket[]>(
        "SELECT COALESCE(SUM(total), 0) as r FROM orders WHERE created_at >= ? AND status = 'PAID' AND payment_method = 'CASH'", [startOfDayFormatted]
    )
    const [onlineRevenueRows] = await pool.execute<mysql.RowDataPacket[]>(
        "SELECT COALESCE(SUM(total), 0) as r FROM orders WHERE created_at >= ? AND status = 'PAID' AND payment_method = 'ONLINE'", [startOfDayFormatted]
    )
...
export async function getDailyReport(date: Date) {
    ...
    const [totalOrdersRows] = await pool.execute<mysql.RowDataPacket[]>(
        'SELECT COUNT(*) as c FROM orders WHERE created_at >= ? AND created_at < ?', [startFormatted, endFormatted]
    )
    const [paidOrdersRows] = await pool.execute<mysql.RowDataPacket[]>(
        "SELECT COUNT(*) as c FROM orders WHERE created_at >= ? AND created_at < ? AND status = 'PAID'", [startFormatted, endFormatted]
    )
    const [cancelledOrdersRows] = await pool.execute<mysql.RowDataPacket[]>(
        "SELECT COUNT(*) as c FROM orders WHERE created_at >= ? AND created_at < ? AND status = 'CANCELLED'", [startFormatted, endFormatted]
    )
    const [totalRevenueRows] = await pool.execute<mysql.RowDataPacket[]>(
        "SELECT COALESCE(SUM(total), 0) as r FROM orders WHERE created_at >= ? AND created_at < ? AND status = 'PAID'", [startFormatted, endFormatted]
    )
